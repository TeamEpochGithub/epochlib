"""This module contains the NumpyArrayToParquetCacher class."""
...
@dataclass
class NumpyArrayToParquetCacher(CacherInterface):
    """The numpy array to .parquet cacher.
    ...
    def load_cache(self, name: str) -> ArrayLike:
        ...
        :param name: The name of the cache, cannot contain characters not in [a-zA-Z0-9_].
        """
        return pd.read_parquet(Path(self.storage_path) / f"{name}.parquet", **self.read_args).to_numpy()
    ...
    def store_cache(self, name: str, data: ArrayLike) -> None:
        ...
        pd.DataFrame(data).to_parquet(storage_path / f"{name}.parquet", **self.store_args)
