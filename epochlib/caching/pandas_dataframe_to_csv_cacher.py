"""This module contains the PandasDataFrameToCSVCacher class."""
...
@dataclass
class PandasDataFrameToCSVCacher(CacherInterface):
    """The pandas dataframe to .csv cacher.

    :param storage_path: The path to store the cache files.
    :param read_args: The arguments to read the cache, pd.read_csv() extra args.
    :param store_args: The arguments to store the cache, pd.to_csv() extra args.
    """
    ...
    def load_cache(self, name: str) -> pd.DataFrame:
        ...
        :param name: The name of the cache, cannot contain characters not in [a-zA-Z0-9_].
        """
        return pd.read_csv(Path(self.storage_path) / f"{name}.csv", **self.read_args)
    ...
    def store_cache(self, name: str, data: pd.DataFrame) -> None:
        ...
        data.to_csv(storage_path / f"{name}.csv", index=False, **self.store_args)
